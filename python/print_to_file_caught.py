...
with open('caught.txt', 'r') as file:
    for line in file:
...
with open('caught_but_not_evolved.txt', 'r') as file:
    for line in file:
...
with open('shiny_locked.txt', 'r') as file:
    for line in file:
...
f = open("formatted_caught.txt", "w")
f.write("")
f.close()

total_boxes = math.ceil(float(len(names)/30))

f = open("formatted_caught.txt", "a", encoding='utf-8')
for current_box in range(1,total_boxes+1):

    # max_length is how many characters a pokemons name is
...
#time.sleep(1)
os.startfile("formatted_caught.txt")